...
from language.enumeration.stock_type import StockType
...
def test_simplest_code():
    ...
    result = executor_manager.execute(
        ...
        """,
        """
        ts_delay(increase_from_lowest_price(low, close, 2), 1) + ts_delay(increase_from_lowest_price(low, close, 2), 2)
        """,
        StockType.ALL,
        date(2017, 1, 1),
        date(2019, 12, 31)
