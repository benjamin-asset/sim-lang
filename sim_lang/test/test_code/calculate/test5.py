...
def add_data_to_day_price(day_price: DataFrame, index_day_price: DataFrame) -> DataFrame:
    ...
    grouped = day_price.groupby("ticker_id")
    for key, group in grouped:
        increase_condition1 = group["pdi_5_sto_pct_change_3"] >= 0.1
...
def back_test(generator, start_date, end_date):
    ...
    # 2. 일봉 가져오기ㄱ
    day_price = generator.get_simulating_data(
        Universe.total,
        [Field.is_active, Field.close, Field.open, Field.high, Field.ticker_id, Field.low],
        start_date,
        end_date,
    )
