...
def docs_to_parsed_data(docs, module_name: str):
    ...
    for node in ast.walk(tree):
        ...
        function_data = Function(node.name, parsed_docstring.short_description.replace(" ", ""), module_name, params)
...
built_in_function_list.extend(docs_to_parsed_data(indicator_text, "indicator"))

function_text = requests.get(function_url).text
built_in_function_list.extend(docs_to_parsed_data(function_text, "function"))

built_in_function_list.append(Function("ts_delay", "정해야함", "language", None))
