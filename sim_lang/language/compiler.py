...
class Call(ast.Call):
    def __init__(self, function, arguments):
        super().__init__(expr_func=function, expr=function, func=function, args=arguments, keywords=[])
...
class Compiler(ast.NodeTransformer):
    ...
    def visit_BinOp(self, node: ast.BinOp) -> Any:
            ...
            return ast.copy_location(
                ast.BinOp(
                    ...
                    Call(
                        ...
                        [
                            ast.Compare(
                                left=self.visit(node.right), ops=[ast.NotEq()], comparators=[ast.Constant(0, 0)]
                            ),
        ...
        return ast.copy_location(ast.BinOp(self.visit(node.left), node.op, self.visit(node.right)), node)
            ...
            for index, argument in enumerate(node.args):
                ...
                if is_enum:
                    enum_node = ast.copy_location(
                        Attribute(Attribute(Name("parameter"), get_enum_name(parameter_type)), argument.id), argument
                    )
